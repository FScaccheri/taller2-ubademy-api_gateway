...
async def get_course_preview(request: Request, course_id: str, current_user: dict = Depends(get_current_user)):
    print("HOLA PREVIEW")
    response = requests.get(BUSINESS_BACKEND_URL + COURSES_PREFIX + "/preview" + f"/{course_id}/{current_user.email}")
    print(response)
    if response.status_code != 200:
        return public_status_messages.get("error_unexpected")
